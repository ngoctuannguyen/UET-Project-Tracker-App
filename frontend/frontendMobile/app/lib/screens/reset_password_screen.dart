...
class ResetPasswordScreen extends StatefulWidget {
  final String oobCode; // Nhận oobCode bắt buộc từ arguments khi điều hướng

  const ResetPasswordScreen({Key? key, required this.oobCode})
    : super(key: key);

  // Định nghĩa route name để dễ dàng điều hướng (nếu bạn dùng named routes)
...
class _ResetPasswordScreenState extends State<ResetPasswordScreen> {
  ...
  // Không cần biến _oobCode riêng vì đã có widget.oobCode

  @override
  void initState() {
    super.initState();
    // Kiểm tra oobCode ngay khi màn hình được khởi tạo (sau frame đầu tiên)
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (widget.oobCode.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Lỗi: Mã đặt lại mật khẩu không hợp lệ hoặc bị thiếu.',
            ),
            backgroundColor: Colors.redAccent,
          ),
        );
        // Tự động quay lại nếu không có code
  ...
  Future<void> _handlePasswordReset() async {
    // Kiểm tra lại oobCode trước khi thực hiện
    if (widget.oobCode.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
    ...
    if (_formKey.currentState!.validate() && !_isLoading) {
      ...
      try {
        ...
        // Điều hướng về màn hình Login sau khi thành công và xóa các màn hình trước đó
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    // Submit Button
                    ElevatedButton(
                      // Vô hiệu hóa nút nếu đang loading hoặc oobCode không hợp lệ
                      onPressed:
                          (_isLoading || widget.oobCode.isEmpty)
                              ? null
