...
rabbitmqConsumer.connect((routingKey, event) => {
    switch (routingKey) {
        case 'event.project.created':
            ...
            if (projectCode && projectName) {
                ...
                console.warn('⚠️ Không tìm thấy projectCode hoặc projectName trong event để thêm product.');
            }
            break;
                ...
                deleteProduct(deletedProjectCode)
                    .then((result) => {
                        if (result) {
                            ...
                            console.error(`❌ Failed to delete product: ${deletedProjectCode}`);
                        }
                    })
                    .catch((error) => {
                        console.error(`❌ Error deleting product: ${deletedProjectCode}`, error);
                    });
            } else {
                console.warn('⚠️ Không tìm thấy projectCode trong event để xóa product.');
            }
            break;
        case 'event.project.task.created':
            // TODO: xử lý thêm nếu muốn
            console.log(event.payload['project_task']);
            const task = event.payload['project_task'][event.payload['project_task'].length - 1];
            ...
            if (Id && description) {
                ...
                console.warn('⚠️ Không tìm thấy Id hoặc description trong event để thêm component.');
            }
        ...
        case 'event.project.task.removed':
                ...
                deleteReportbyComponentId(taskId)
                    .then((result) => {
                        if (result.success) {
                            ...
                            console.error(`❌ Failed to delete report for taskId: ${taskId}`, result.message);
                        }
                    })
                    .catch((error) => {
                        console.error(`❌ Error deleting report for taskId: ${taskId}`, error);
                    });
          ...
          case 'event.project.task.updated':
            ...
            const updatedTaskId = event.payload['taskId'] || event.data?.taskId || event.componentCode;
            const updatedStatus = event.payload['data']['project_task'].find(task => task.task_id === updatedTaskId)?.status || event.payload['data']['project_task'].find(task => task.componentCode === updatedTaskId)?.status;
                ...
                updateStatus(updatedTaskId, updatedStatus)
                    .then((result) => {
                        if (result) {
                            ...
                            console.error(`❌ Failed to update status for taskId: ${updatedTaskId}`);
                        }
                    })
                    .catch((error) => {
                        console.error(`❌ Error updating status for taskId: ${updatedTaskId}`, error);
                    });
                updateProgress(updatedTaskId)
                    .then((result) => {
                        if (result.success) {
                            ...
                            console.error(`❌ Failed to update progress for taskId: ${updatedTaskId}`, result.message);
                        }
                    })
                    .catch((error) => {
                        console.error(`❌ Error updating progress for taskId: ${updatedTaskId}`, error);
                    });
            } else {
                console.warn('⚠️ Không tìm thấy taskId trong event để cập nhật tiến độ.');
            }
