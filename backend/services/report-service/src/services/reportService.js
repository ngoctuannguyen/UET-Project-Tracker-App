...
const getReportsByProductId = async (productCode) => {
  try {
    ...
    if (!reports) {
       return { success: false, message: 'No reports found for this product' };
    }
...
const getEmployeePerformanceService = async (employeeId) => {
  try {
    const reports = await Report.findAll({
      where: { employeeId },
    });
    if (!reports) return { success: false, message: 'No reports found for this employee' };
    return { success: true, reports };
  } catch (error) {
...
module.exports = {submitReport, scanBarcodeFromImage,getReportsByProductId,getAllReports,deleteReport,deleteReportbyProductId,getEmployeePerformanceService};
