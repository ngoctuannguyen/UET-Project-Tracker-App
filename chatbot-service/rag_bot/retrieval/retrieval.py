...
class HybridSearch(BaseRetriever):
    ...
    def __init__(self, qdrant_client, embedding_model: HuggingFaceBgeEmbeddings, sparse_embedding_model, reranker) -> None:
        """
    ...
    def get_relevant_documents(self, query, documents):
        ...
        scores = self.model.score([(query, doc) for doc in documents])
...
if __name__ == "__main__":
    ...
    qdrant_client = QdrantVectorStore("my_collection", embedding_model).get_client()
    hybrid = HybridSearch(qdrant_client, embedding_model, sparse_embedding_model)
    initial_docs = hybrid.query_hybrid_search("What is a MAC address?")
    if initial_docs is None or len(initial_docs) == 0:
        print("No documents found.")
    else:
        re_ranker = ReRanking(ReRanker().get_reranker())
