...
class Project {
  ...
  static project_progress(taskData) {
    ...
    if (
      taskData.start_date &&
      admin.firestore.Timestamp.fromDate(new Date(taskData.start_date)) >
  ...
  static check_project_status(projectProgress, projectData) {
    ...
    for (const entry of projectProgress) {
      if (
        entry.start_date &&
        (!min_start_date || entry.start_date < min_start_date)
      ) {
  ...
  static async create(projectData) {
    ...
    await projectRef.set({
      ...
      project_due: admin.firestore.Timestamp.fromDate(
        new Date(projectData.project_due)
      ),
  ...
  static async update_project_task(projectId, task_id, updatedData) {
    ...
    updatedData.start_date = admin.firestore.Timestamp.fromDate(
      new Date(updatedData.start_date)
    );
    updatedData.deadline = admin.firestore.Timestamp.fromDate(
      new Date(updatedData.deadline)
    );
  ...
  static async remove_employee(projectId, employeeId) {
    ...
    try {
      await admin.firestore().runTransaction(async (transaction) => {
        ...
        const newEmployeeList = projectDoc
          .data()
          .employee_list.filter((id) => id !== employeeId);
  ...
  static async getAll() {
    ...
    const projects = projectsSnapshot.docs.map((doc) => {
      ...
      return {
        ...
        project_due:
          Project.convertFirebaseTime(projectData.project_due) || "N/A",
        created_at:
          Project.convertFirebaseTime(projectData.created_at) || "N/A",
      };
    });
  ...
  static async getByLeader(project_leader) {
    const projectsSnapshot = await project_service
      .where("project_leader", "==", project_leader)
      .get();
    const projects = projectsSnapshot.docs.map((doc) => {
      const projectData = doc.data();
      ...
      return {
        ...
        project_due:
          Project.convertFirebaseTime(projectData.project_due) || "N/A",
        created_at:
          Project.convertFirebaseTime(proojectData.created_at) || "N/A",
      };
    });
    return projects;
  }
  ...
  static async getByEmployee(employeeId) {
    const projectsSnapshot = await project_service
      .where("employee_list", "array-contains", employeeId)
      .get();
    const projects = projectsSnapshot.docs.map((doc) => {
      const projectData = doc.data();
  ...
  static async updateTaskStatusFromReport(
    projectId,
    taskId,
    newStatusFromReport
  ) {
    // projectId là productCode từ report-service
    // taskId là componentCode từ report-service
    // newStatusFromReport là giá trị của is_completed từ report-service ("not started", "in progress", "done")
    ...
    const taskIndex = projectTasks.findIndex((task) => task.task_id === taskId);
    ...
    // Chỉ cập nhật trường status của task
    // Đảm bảo newStatusFromReport là một trong các giá trị hợp lệ cho trường status của Firebase
    if (["not started", "in progress", "done"].includes(newStatusFromReport)) {
      if (projectTasks[taskIndex].status !== newStatusFromReport) {
        projectTasks[taskIndex].status = newStatusFromReport;

        // Cập nhật lại toàn bộ mảng project_task trong Firestore
        await projectRef.update({
          project_task: projectTasks,
        });
        console.log(
          `[Project Model] Task ${taskId} in project ${projectId} status updated to "${newStatusFromReport}" from report.`
        );
      } else {
        console.log(
          `[Project Model] Task ${taskId} in project ${projectId} status is already "${newStatusFromReport}". No update needed from report.`
        );
      }
    } else {
      console.warn(
        `[Project Model] Invalid status "${newStatusFromReport}" received for task ${taskId}. Status not updated.`
      );
      // Có thể throw lỗi nếu muốn việc cập nhật phải diễn ra với giá trị hợp lệ
    }
